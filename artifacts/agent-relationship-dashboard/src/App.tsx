...
function QueryError({ onRetry, message = "The relationship picture could not load." }: { onRetry: () => void; message?: string }) {
  return <div className="state-panel error-state" data-testid="state-error"><div className="state-icon"><AlertCircle size={24} /></div><h3>Could not open the picture</h3><p>{message}</p><button className="button button-secondary" onClick={onRetry} data-testid="button-retry"><RefreshCw size={15} /> Try again</button></div>;
}
...
function ActivityFeed({ activities, loading, onSelectAgent }: { activities: RelationshipActivity[]; loading: boolean; onSelectAgent: (id: string) => void }) {
  ...
  if (!activities.length) return <EmptyState title="A quiet moment" detail="New relationship activity will appear here as conversations are reviewed." />;
  ...
  return <div className="dashboard-page">
    ...
    {(summaryQuery.isError || agentsQuery.isError || activityQuery.isError) ? <QueryError onRetry={refresh} /> : <>
      ...
      <div className="dashboard-grid top-grid">
        <section className="surface next-actions" data-testid="section-next-actions"><div className="surface-heading"><div><p className="section-kicker">Attention ledger</p><h2>Next actions</h2></div><span className="count-badge">{priorityAgents.length} to review</span></div><p className="surface-intro">A short list of relationships where a thoughtful nudge can change the week.</p>{loading ? <div className="priority-list">{[1, 2, 3].map((item) => <div className="priority-row" key={item}><SkeletonBlock className="priority-avatar" /><div className="priority-copy"><SkeletonBlock className="line-skeleton medium" /><SkeletonBlock className="line-skeleton long" /></div></div>)}</div> : priorityAgents.length ? <div className="priority-list">{priorityAgents.map((agent) => <button className="priority-row" key={agent.id} onClick={() => setSelectedAgent(agent)} data-testid={`priority-${agent.id}`}><div className={`priority-avatar ${agent.priority === "high" ? "priority-high" : "priority-medium"}`}>{agent.priority === "high" ? "!" : "→"}</div><div className="priority-copy"><div className="priority-heading"><strong>{agent.name}</strong><StatusPill value={agent.relationshipHealth} /></div><p>{agent.nextAction}</p><span>{agent.studentName !== "—" ? agent.studentName : agent.groupName} · {formatDate(agent.lastActivityDate)}</span></div><ChevronRight size={17} className="row-chevron" /></button>)}</div> : <EmptyState title="No immediate follow-ups" detail="Your attention ledger is clear for now." />}</section>
        <section className="surface pulse-card" data-testid="section-portfolio-pulse"><div className="surface-heading"><div><p className="section-kicker">Portfolio pulse</p><h2>What changed in the room</h2></div><Activity size={19} className="surface-heading-icon" /></div><div className="pulse-stat"><div className="pulse-number">{summary?.recentReferralCount ?? 0}</div><div><strong>recent referrals</strong><p>Conversations with movement in the latest review.</p></div></div><div className="pulse-divider" /><div className="pulse-stat"><div className="pulse-number pulse-number-dark">{summary?.closedReferralCount ?? 0}</div><div><strong>closed with an outcome</strong><p>Useful signal for the next agent conversation.</p></div></div><div className="pulse-callout"><Bell size={15} /><span>Protect the warm handoffs before chasing new ones.</span></div></section>
      </div>
